...
エッジAIは、クラウドAIを置き換えるものではありません。現実的には、**つながっている時はクラウドで最新情報を取り込み、通信が弱い時はローカルで必要な判断支援を続ける**ハイブリッド構成が重要です。
...
その中核の一つが、**Edge Node**という考え方です。Edge Nodeは、機内・船内・車内などに置く小型のローカル実行環境です。すべてをクラウドに問い合わせるのではなく、必要な情報を事前に保持し、通信状態に応じて表示・更新する構成を想定しています。
